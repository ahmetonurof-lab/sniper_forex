...
def mk(trade_id, entry, exit, pnl, result='TP', symbol='TEST'):
    return BenchmarkTrade(
        ...
        exit_price=1.0 + pnl * 0.01, exit_bar_index=0,
        exit_timestamp=exit, result=result, pnl_r=pnl,
    )
...
for trades, entries in orders:
    ...
    mults = sorted([(t.trade_id, round(t.pnl_r / t.pnl_r, 4)) for t in trades for s_obj in s if s_obj.trade_id == t.trade_id])
