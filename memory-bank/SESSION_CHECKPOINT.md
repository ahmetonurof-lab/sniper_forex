...
Piece-1 tamam (9+ commit remote'ta). TAG `research-canonical-v1.1` = fcb9b88 (annotated) → peel **7a1e6f1** (executable freeze; C2-wire commit) — doğrulandı, GEÇERLİ (time-semantics sorusu CBDR-alignment raporuyla kapandı; §6.3/DST kış-probe açık-soru ③). SRI-001 tamam: **Chain-6 GO, Chain-4 RED** (hakem arbitraj hükmü; owner onayı Forexçi'de). **N2 #16 push TAMAM** (remote HEAD `5b23da5`; yetkili-set {20f3510, 5b23da5}; imzalar yeşil; `3acb7cd`+`665fb17` defter-commit'leri set-dışı → sonraki yetkili-push'un yükü). Soak: **T0#5 DOWN** (17:50 lock-rename WinError 5 → K1 8-attempt tükenişi → SHUTDOWN exit=1) · **T0#6 DOWN** (18:14 boot PROCEED; **exclusion-AKTİF-iken** lock-rename WinError 5 TEKRAR (PID 15784, log-kanıtı) → reconnect→PROCEED → ≤18:23 ölü) → **exclusion lock-path'i tek-başına ÇÖZMEDİ; iki-boot-özdeş-imza.** **HAKEM HÜKMÜ (2026-09-02): exclusion-hipotezi FALSİFİYE; en-güçlü-aday dual-process lock-write çatışması; N2 #17 spec (ANA: lock rename→in-place write+fdatasync; EK: dual-instance pre-guard) ONAYLANDI → İCRA EDİLDİ** (aynı oturum; wave1 65/65; commit-başlığı N2 #17). T0#7 = N2 #17 sonrası boot; boot-crash-log'daki `writer_diagnostic` satırları dual-instance ölçümünü verir (2×run_production-argv = CONFIRMED). Chain-6 entegrasyonu DONDURULMUŞ kalıyor (N2 #17→T0#7→SOAK→sonra). Canlı-CBDR penceresi 19:00 UTC — sistem DOWN; push `3acb7cd`+`665fb17`+N2#17-commit Hakem-hash-bound-yetkisi bekliyor.
...
**B) N2 #16 checkpoint-commit + push:** `814c8f8` (pushlanmamış ancestor) + bu checkpoint + SRI-001 artifact'ları (§10 aday-set) — Hakem hash-bound onayı sonrası. → **İCRA-NOTU (2026-09-02):** KAPANDI — set remote'a girdi (`44d99a1..892b52d`, ardından `..3eaf7e7` N2 #15-b); push-kayıtları + imzalar progress.md'de (N2 #16 PUSH KAYDI + N2 #15-b PUSH KAYDI satırları).
...
- **T0#5/T0#6 NÜKS (2026-09-02 akşam):** T0#5: 17:50 WRITE_BLOCK (retries=8 tükeniş) → SHUTDOWN exit=1 → shutdown_snapshot ERROR. T0#6 (exclusion-AKTİF, 5007-OS-kanıtlı): lock-rename WinError 5 **yine** (PID 15784) → reconnect-ladder PROCEED → ölüm ≤18:23, terminal-audit-event'ler flushed-olmadan kayboldu (buffer-hipotezi — DOĞRULANMADI). İki-boot-özdeş-imza → rastgele-AV-yarışı DEĞİL-intiba; exclusion lock-path-rename'i tek-başına çözmedi. Cline-probe-dersi: head/tail-pencereleri crash-satırını gizledi — sağlık-probu TAM-log ile yap (§13.5-dürüstlük).
- **N2 #17 İCRASI (2026-09-02, Hakem hükmü: dual-process lock-write çatışması en-güçlü-aday):** (1) `Lock._write` IN-PLACE: os.open(O_WRONLY|O_CREAT)→ftruncate→write→fsync (fdatasync POSIX-only; fsync Windows-eşdeğeri) — tmp/rename lock-hot-path'ten ÇIKTI; K1 8-attempt ladder + K3 WRITE_BLOCK-once + D35 exhaustion-fatal KORUNDU; parent-mkdir sözleşmesi korundu. (2) `_atomic_write_text` (audit/state/safe-mode) rename-KALICI (atomiklik-gereksinimi); tükeniş → K2 `state/crash_log.txt` append (flush-bağımsız-kanıt-katı; `_ATOMIC_WRITE_RUNTIME=False` donuk-postür — test-pinned, sessiz-flip-yasak). (3) `run_production.main()` dual-instance pre-guard (Hakem EK-FİX): lock-exists + PID-alive → "[run_production] Already running ... - EXIT" → 0 (MT5-öncesi; stale/corrupt/dead-PID canonical-takeover'da-kalır). (4) `writer_diagnostic` crash-log satırı (one-shot: pid+argv+parent-image) — **T0#7 ölçüm-kuralı: tek-boot-crash-log'da İKİ run_production-argv'li satır = dual-instance CONFIRMED; tek = hipotez-DÜŞER.**
...
- **Bilinen-untracked-sınıfları (kasıtlı, dokunulmadı):** `docs/` (audit-raporları) · `data/` · `state/` (soak-runtime + t05/t06 boot-logları) · `src/backtest/` + `tests/test_p1_*` (önceden-var, önceki oturumlar) · `tools/*.QUARANTINED_20260901` (D53b karantina) · `results/research/` · `scripts/`, `tools/` yardımcıları (incl. `tools/t0_5_exclusion.ps1` + `tools/t0_5_elevate.ps1` — UAC-deneysel)
