...
def probe_symbol(data_layer, symbol_name):
    ...
    for tf in timeframes:
        ...
        if rates and len(rates) > 0:
            ...
            result["ohlc"][tf] = {"count": 0, "quality": "unavailable"}
    ...
    if result["tick"]:
        ...
        if bid_ask_ok:
            quality_results.append("[OK] bid/ask valid")
        else:
            quality_results.append("[WARN] bid/ask inverted")
...
def main():
    ...
    try:
        ...
        if not conn.connect():
            ...
            print(
                f"   Details: {os.getenv('MT5_LOGIN', 'login')} / {os.getenv('MT5_SERVER', 'server')}"
            )
            sys.exit(1)
        ...
        print("   .env file may not be loading correctly")
        sys.exit(1)
    ...
    for symbol in available:
        ...
        if result["available"]:
            ...
            print("  [WARN] Symbol not available")
    ...
    for tf in ["M1", "M5", "M15", "H1"]:
        ...
        if count > 0:
            ...
            print(
                f"      {tf:4s} : {count}/10 bars, first={first}, last={last}, latest_close={latest}"
            )
        else:
