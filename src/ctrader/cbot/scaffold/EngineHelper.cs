...
internal static class EngineHelper
{
    ...
    internal static void Run(Action action, Action<Exception> exceptionHandler)
    {
        try
        {
            action();
...
");
    ...
    {
        // Robust: match an actual class definition (class Name followed by '(' or ':')
        // at line start — NOT the first "class" token anywhere (docstrings/comments
        // can contain the word "class" and break the naive split-based parser).
