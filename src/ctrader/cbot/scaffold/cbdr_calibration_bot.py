...
#                    arrives via the cAlgo.API star import)
...
# --- import core.py (module name resolved by in-memory importer) ---
...
from System import DateTime, DateTimeKind, TimeSpan  # noqa: E402
...
class CalibrationBot:
    ...
    def on_start(self):
        ...
        self._last_ts = None  # epoch-minutes of last ingested bar (dedup guard)

        # Warm up from already-CLOSED history bars so the first live bar has
        # ATR + body context (deterministic reconstruction, §6.2). The last
        # bar (Count-1) is still FORMING at OnStart — exclude it; on_bar will
        # feed it exactly once after it closes.
        if api.Bars.Count > 1:
            for i in range(api.Bars.Count - 1):
                self._feed_bar(i)
    ...
    def on_bar(self):
        # OnBar fires when a NEW bar OPENS (official BarOpened Example reads
        # api.Bars[Count - 2] as the previous bar). The just-CLOSED bar is
        # therefore Count - 2 — feed that, never the still-forming Count - 1.
        if api.Bars.Count >= 2:
            self._feed_bar(api.Bars.Count - 2)
        self._draw_panel()
    ...
    def _feed_bar(self, idx: int):
        bar = api.Bars[idx]
        minute = self._epoch_minutes(bar.OpenTime)  # Bar exposes OpenTime
        if minute == self._last_ts:
            return  # duplicate event (double fire / overlap guard)
        self._last_ts = minute
        mb = MiniBar(
            ts_minutes=minute,
    ...
    def _epoch_minutes(dt) -> int:
        """Bar OpenTime (System.DateTime) -> integer epoch-minutes.

        Timezone convention (contract 6.3): cTrader bar times are UTC
        wall-clock; read the wall-clock directly. ToUniversalTime() is
        deliberately NOT used - on a Kind=Unspecified DateTime it applies
        the OS-local offset (this host: UTC+3) and would silently shift
        every CBDR window by three hours.
        """
        epoch = DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc)
        return int(dt.Subtract(epoch).TotalMinutes)
