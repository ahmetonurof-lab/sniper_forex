...
class Orchestrator:
    ...
    def _run_phases(self) -> StartupResult:
        ...
        if self._lifecycle is not None:
            try:
                ...
                safe_reasons.append(
                    f"lifecycle_recovery_failed: {type(e).__name__}"
                )
        runtime_warmed_at_s7 = bool(
            getattr(self._runtime, "_warmed", False)
        )
        ...
        self._restored = bool(
            self._runtime_restored or self._lifecycle_restored
        )
    ...
    def _warmup(self, m1_count: int) -> Tuple[bool, int, dict]:
        ...
        warm_skip = bool(
            ...
            and getattr(self._runtime, "_warmed", False)
        )
    ...
    def _fetch_m1_tri_state(self, count: int) -> Tuple[str, Any]:
        ...
        """
        try:
            if self._mt5_conn is not None and hasattr(
                self._mt5_conn, "get_rates"
            ):
                rates = self._mt5_conn.get_rates(self._symbol, "M1", count)
                if rates is None:
                    err = getattr(
                        self._mt5_conn, "last_error", "rates_unavailable"
                    )
